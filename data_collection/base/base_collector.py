...
class base_collector(ABC):
    def __init__(self, name:str, config: Dict[str, Any]):
        self.name = name
        self.config = config
        self.logging = logging.getLogger(f'{__name__}.{name}')
        self._setup_logging()
    ...
    async def process(self, symbol: str, strat_date: date_time,
                      end_date: datetime) -> List[stock_data]:
...
try:
            ...
            self.logger.info(f'Sucessfully collected {len(validated_data)} records')
    ...
    def _is_valid_record(self, record:stock_data) -> bool:
        ...
        if any(v is None for v in [record.open, record.high, 
